...
export function FontSizeToolbarButton() {
  const [inputValue] = React.useState(DEFAULT_FONT_SIZE);
  ...
  return (
    <div className="flex h-7 items-center gap-1 rounded-md bg-muted/60 p-0">
      ...
      <Popover open={isFocused} onOpenChange={setIsFocused} modal={false}>
        <PopoverTrigger
          render={
            <button
              onClick={() => setIsFocused(true)}
              className="h-full w-10 shrink-0 bg-transparent px-1 text-center text-sm hover:bg-muted rounded-md">
              <span>{displayValue}</span>
            </button>
          }>
          <span>{displayValue}</span>
        </PopoverTrigger>
        <PopoverPopup className="w-10 px-px py-1">
          {FONT_SIZES.map((size) => (
            <button
              key={size}
              className={cn(
